...
class InvertedPendulum:
    ...
    def __init__(self):
        ...
        self.x_dot = rospy.get_param("~x_dot", 0.0)
        self.x_d_dot = rospy.get_param("~x_d_dot", 0.0)
        self.theta = rospy.get_param("~theta", 0.0)
        self.theta_dot = rospy.get_param("~theta_dot", 0.0)
        self.theta_d_dot = rospy.get_param("~theta_d_dot", 0.0)
        self.length = rospy.get_param("~length", 100.0)
        self.cart_m = rospy.get_param("~cart_m", 0.5)
        self.pendulum_m = rospy.get_param("~pendulum_m", 0.5)
        ...
        print(f"self.x_dot = {self.x_dot}")
        print(f"self.x_d_dot = {self.x_d_dot}")
        print(f"self.theta = {self.theta}")
        print(f"self.theta_dot = {self.theta_dot}")
        print(f"self.theta_d_dot = {self.theta_d_dot}")
        print(f"self.length = {self.length}")
        print(f"self.cart_m = {self.cart_m}")
        print(f"self.pendulum_m = {self.pendulum_m}")
        print("--------------------------------------------")
